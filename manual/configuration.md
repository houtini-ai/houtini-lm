...
| `HOUTINI_LM_FALLBACK_MODELS` | *(none)* | OpenRouter only. Comma-separated model ids tried, in order, when the primary model errors — rate limit, downtime, moderation refusal or context-length validation. Maps to OpenRouter's [`models` routing](https://openrouter.ai/docs/guides/routing/model-fallbacks); other backends ignore it. The primary model is attempt one, so leave it out of the list. Billing follows whichever model served the request, and its id comes back in the response `model` field, which the footer reports — so keep paid ids out of the list if you are staying on a free tier. |
