...
- `json_schema` - force structured output. Pass a JSON Schema and the response is JSON conforming to it. The reliable way to get parseable data out of a small model - much better than asking nicely in the prompt. OpenAI's strict mode wants `additionalProperties: false` on every object and every property listed in `required`; houtini-lm adds the first for you, and sends a schema with optional properties non-strict rather than rewriting it. Backends without schema support (DeepSeek) get one retry in plain JSON mode with the schema written into the prompt, which gives valid JSON that usually, though not provably, conforms.
